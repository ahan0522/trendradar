...
export function HomeHeroSignal() {
  ...
  const isMonthlyCandidate = data?.source === "monthly_current" || heroSignal?.id.startsWith("monthly-");
      ...
      <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
        <div>
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            {isMonthlyCandidate ? (
              <span className="rounded-full border border-sky-300/30 bg-sky-400/10 px-6 py-3 text-sm font-black text-sky-200">
                當月候選，等待正式驗證
              </span>
            ) : (
          ...
          <div className="rounded-3xl border border-zinc-800 bg-zinc-950/80 p-5">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="rounded-full border border-zinc-700 bg-zinc-900 px-3 py-1 text-xs font-bold text-zinc-300">
                {isMonthlyCandidate ? "當月候選" : isDerivedPreview ? "主題推導中" : "Signal Ledger"}
              </span>
            </div>
