import type { NewsSource } from "@/types/news";

export const rssSources: NewsSource[] = [
  {
    id: "google-news-tw-top",
    name: "Google News 台灣熱門",
    category: "新聞",
    region: "台灣",
    url: "https://news.google.com/rss?hl=zh-TW&gl=TW&ceid=TW:zh-Hant",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "aggregator",
    sourceTier: "secondary_analysis",
    sourceWeight: 0.8,
    credibilityWeight: 0.75,
    role: "volume_signal",
  },
  {
    id: "google-news-world",
    name: "Google News 國際",
    category: "國際",
    region: "全球",
    url: "https://news.google.com/rss/headlines/section/topic/WORLD?hl=zh-TW&gl=TW&ceid=TW:zh-Hant",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "aggregator",
    sourceTier: "secondary_analysis",
    sourceWeight: 0.8,
    credibilityWeight: 0.75,
    role: "volume_signal",
  },
  {
    id: "google-news-business",
    name: "Google News 財經",
    category: "財經",
    region: "台灣 / 全球",
    url: "https://news.google.com/rss/headlines/section/topic/BUSINESS?hl=zh-TW&gl=TW&ceid=TW:zh-Hant",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "aggregator",
    sourceTier: "secondary_analysis",
    sourceWeight: 0.8,
    credibilityWeight: 0.75,
    role: "volume_signal",
  },
  {
    id: "google-news-sports",
    name: "Google News 體育",
    category: "體育",
    region: "台灣 / 全球",
    url: "https://news.google.com/rss/headlines/section/topic/SPORTS?hl=zh-TW&gl=TW&ceid=TW:zh-Hant",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "aggregator",
    sourceTier: "secondary_analysis",
    sourceWeight: 0.8,
    credibilityWeight: 0.75,
    role: "volume_signal",
  },
  {
    id: "google-news-technology",
    name: "Google News 科技",
    category: "3C",
    region: "台灣 / 全球",
    url: "https://news.google.com/rss/headlines/section/topic/TECHNOLOGY?hl=zh-TW&gl=TW&ceid=TW:zh-Hant",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "aggregator",
    sourceTier: "secondary_analysis",
    sourceWeight: 0.8,
    credibilityWeight: 0.75,
    role: "volume_signal",
  },
  {
    id: "cna-world",
    name: "中央社 國際",
    category: "國際",
    region: "台灣 / 全球",
    url: "https://feeds.feedburner.com/rsscna/intworld",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "mainstream_news",
    sourceTier: "secondary_analysis",
    sourceWeight: 1.15,
    credibilityWeight: 1.25,
    role: "fact_verification",
  },
  {
    id: "cna-politics",
    name: "中央社 政治",
    category: "新聞",
    region: "台灣",
    url: "https://feeds.feedburner.com/rsscna/politics",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "mainstream_news",
    sourceTier: "secondary_analysis",
    sourceWeight: 1.15,
    credibilityWeight: 1.25,
    role: "fact_verification",
  },
  {
    id: "cna-cross-strait",
    name: "中央社 兩岸",
    category: "國際",
    region: "台灣 / 中國",
    url: "https://feeds.feedburner.com/rsscna/mainland",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "mainstream_news",
    sourceTier: "secondary_analysis",
    sourceWeight: 1.15,
    credibilityWeight: 1.25,
    role: "fact_verification",
  },
  {
    id: "cna-social",
    name: "中央社 社會",
    category: "新聞",
    region: "台灣",
    url: "https://feeds.feedburner.com/rsscna/social",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "mainstream_news",
    sourceTier: "secondary_analysis",
    sourceWeight: 1.15,
    credibilityWeight: 1.25,
    role: "fact_verification",
  },
  {
    id: "cna-life",
    name: "中央社 生活",
    category: "生活",
    region: "台灣",
    url: "https://feeds.feedburner.com/rsscna/lifehealth",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "mainstream_news",
    sourceTier: "secondary_analysis",
    sourceWeight: 1.1,
    credibilityWeight: 1.2,
    role: "fact_verification",
  },
  {
    id: "cna-local",
    name: "中央社 地方",
    category: "生活",
    region: "台灣",
    url: "https://feeds.feedburner.com/rsscna/local",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "local_news",
    sourceTier: "secondary_analysis",
    sourceWeight: 1,
    credibilityWeight: 1.15,
    role: "fact_verification",
  },
  {
    id: "cna-culture",
    name: "中央社 文化",
    category: "文化",
    region: "台灣 / 全球",
    url: "https://feeds.feedburner.com/rsscna/culture",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "mainstream_news",
    sourceTier: "secondary_analysis",
    sourceWeight: 1,
    credibilityWeight: 1.15,
    role: "fact_verification",
  },
  {
    id: "cna-entertainment",
    name: "中央社 娛樂",
    category: "娛樂",
    region: "台灣 / 全球",
    url: "https://feeds.feedburner.com/rsscna/stars",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "mainstream_news",
    sourceTier: "secondary_analysis",
    sourceWeight: 0.95,
    credibilityWeight: 1.1,
    role: "fact_verification",
  },
  {
    id: "cna-finance",
    name: "中央社 產經證券",
    category: "財經",
    region: "台灣",
    url: "https://feeds.feedburner.com/rsscna/finance",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "mainstream_news",
    sourceTier: "secondary_analysis",
    sourceWeight: 1.15,
    credibilityWeight: 1.25,
    role: "fact_verification",
  },
  {
    id: "cnyes-tw-stock",
    name: "鉅亨網 台股",
    category: "財經",
    region: "台灣",
    url: "https://m.cnyes.com/news/cat/tw_stock?r=rss",
    enabled: false,
    sourcePool: "news_media",
    sourceKind: "industry_media",
    sourceTier: "secondary_analysis",
    sourceWeight: 0.75,
    credibilityWeight: 0.9,
    role: "fact_verification",
  },
  {
    id: "ltn-politics",
    name: "自由時報 政治",
    category: "新聞",
    region: "台灣",
    url: "https://news.ltn.com.tw/rss/politics.xml",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "mainstream_news",
    sourceTier: "secondary_analysis",
    sourceWeight: 1.05,
    credibilityWeight: 1.1,
    role: "fact_verification",
  },
  {
    id: "ltn-world",
    name: "自由時報 國際",
    category: "國際",
    region: "台灣 / 全球",
    url: "https://news.ltn.com.tw/rss/world.xml",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "mainstream_news",
    sourceTier: "secondary_analysis",
    sourceWeight: 1.05,
    credibilityWeight: 1.1,
    role: "fact_verification",
  },
  {
    id: "ltn-life",
    name: "自由時報 生活",
    category: "生活",
    region: "台灣",
    url: "https://news.ltn.com.tw/rss/life.xml",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "mainstream_news",
    sourceTier: "secondary_analysis",
    sourceWeight: 1,
    credibilityWeight: 1.05,
    role: "fact_verification",
  },
  {
    id: "ltn-sports",
    name: "自由體育",
    category: "體育",
    region: "台灣 / 全球",
    url: "https://news.ltn.com.tw/rss/sports.xml",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "mainstream_news",
    sourceTier: "secondary_analysis",
    sourceWeight: 1.05,
    credibilityWeight: 1.1,
    role: "fact_verification",
  },
  {
    id: "ttv-news",
    name: "台視新聞",
    category: "新聞",
    region: "台灣",
    url: "https://www.ttv.com.tw/rss/RSSHandler.ashx?d=news",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "mainstream_news",
    sourceTier: "secondary_analysis",
    sourceWeight: 1,
    credibilityWeight: 1.05,
    role: "fact_verification",
  },
  {
    id: "pts-news",
    name: "公視新聞",
    category: "新聞",
    region: "台灣",
    url: "https://news.pts.org.tw/xml/newsfeed.xml",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "mainstream_news",
    sourceTier: "secondary_analysis",
    sourceWeight: 1.08,
    credibilityWeight: 1.2,
    role: "fact_verification",
  },
  {
    id: "ettoday-news",
    name: "ETtoday新聞雲",
    category: "新聞",
    region: "台灣",
    url: "http://feeds.feedburner.com/ettoday/newslist",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "mainstream_news",
    sourceTier: "secondary_analysis",
    sourceWeight: 0.95,
    credibilityWeight: 1,
    role: "fact_verification",
  },
  {
    id: "bbc-chinese",
    name: "BBC中文",
    category: "國際",
    region: "全球",
    url: "https://feeds.bbci.co.uk/zhongwen/trad/rss.xml",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "mainstream_news",
    sourceTier: "secondary_analysis",
    sourceWeight: 1.05,
    credibilityWeight: 1.15,
    role: "fact_verification",
  },
  {
    id: "cna-technology",
    name: "中央社 科技",
    category: "科技",
    region: "台灣 / 全球",
    url: "https://feeds.feedburner.com/rsscna/technology",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "mainstream_news",
    sourceTier: "secondary_analysis",
    sourceWeight: 1.15,
    credibilityWeight: 1.25,
    role: "fact_verification",
  },
  {
    id: "technews",
    name: "科技新報",
    category: "科技",
    region: "台灣 / 全球",
    url: "https://technews.tw/feed/",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "industry_media",
    sourceTier: "secondary_analysis",
    sourceWeight: 1.05,
    credibilityWeight: 1.1,
    role: "fact_verification",
  },
  {
    id: "mofa-news",
    name: "外交部 新聞稿",
    category: "台海",
    region: "台灣 / 全球",
    url: "https://www.mofa.gov.tw/OpenData.aspx?SN=E0AB271EE713F7E8",
    enabled: true,
    sourcePool: "official_source",
    sourceKind: "official_announcement",
    sourceTier: "primary_source",
    sourceWeight: 0.65,
    credibilityWeight: 1.5,
    role: "official_record",
  },
  {
    id: "mofa-statements",
    name: "外交部 重要聲明",
    category: "台海",
    region: "台灣 / 全球",
    url: "https://www.mofa.gov.tw/OpenData.aspx?SN=EF2694551B4BF122",
    enabled: true,
    sourcePool: "official_source",
    sourceKind: "official_announcement",
    sourceTier: "primary_source",
    sourceWeight: 0.65,
    credibilityWeight: 1.5,
    role: "official_record",
  },
  {
    id: "mofa-reference",
    name: "外交部 即時新聞澄清與參考資料",
    category: "台海",
    region: "台灣 / 全球",
    url: "https://www.mofa.gov.tw/OpenData.aspx?SN=8ABA444B9AC1360F",
    enabled: true,
    sourcePool: "official_source",
    sourceKind: "official_announcement",
    sourceTier: "primary_source",
    sourceWeight: 0.65,
    credibilityWeight: 1.5,
    role: "official_record",
  },
  {
    id: "finance-technews",
    name: "科技新報 財經",
    category: "財經",
    region: "台灣 / 全球",
    url: "https://finance.technews.tw/feed/",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "industry_media",
    sourceTier: "secondary_analysis",
    sourceWeight: 1.05,
    credibilityWeight: 1.05,
    role: "fact_verification",
  },
  {
    id: "cna-sports",
    name: "中央社 運動",
    category: "體育",
    region: "台灣 / 全球",
    url: "https://feeds.feedburner.com/rsscna/sport",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "mainstream_news",
    sourceTier: "secondary_analysis",
    sourceWeight: 1.15,
    credibilityWeight: 1.25,
    role: "fact_verification",
  },
  {
    id: "the-verge",
    name: "The Verge",
    category: "3C",
    region: "全球",
    url: "https://www.theverge.com/rss/index.xml",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "industry_media",
    sourceTier: "secondary_analysis",
    sourceWeight: 1.05,
    credibilityWeight: 1.1,
    role: "fact_verification",
  },
  {
    id: "techcrunch",
    name: "TechCrunch",
    category: "AI",
    region: "全球",
    url: "https://techcrunch.com/feed/",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "industry_media",
    sourceTier: "secondary_analysis",
    sourceWeight: 1.05,
    credibilityWeight: 1.1,
    role: "fact_verification",
  },
  {
    id: "polygon",
    name: "Polygon",
    category: "遊戲",
    region: "全球",
    url: "https://www.polygon.com/rss/index.xml",
    enabled: true,
    sourcePool: "news_media",
    sourceKind: "industry_media",
    sourceTier: "secondary_analysis",
    sourceWeight: 1.05,
    credibilityWeight: 1.1,
    role: "fact_verification",
  },
];
