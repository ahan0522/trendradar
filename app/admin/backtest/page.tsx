      ...
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <section className="rounded-3xl border border-zinc-800 bg-zinc-950/90 p-6">
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <button onClick={() => runBacktest(false)} disabled={loading || !signalEventId} className="rounded-2xl bg-white px-6 py-3 font-black text-zinc-950 transition hover:bg-sky-100 disabled:cursor-not-allowed disabled:opacity-50">
              Run Selected 7/14/30/60D
            </button>
            <button onClick={() => runBacktest(true)} disabled={loading} className="rounded-2xl border border-zinc-700 bg-zinc-900 px-6 py-3 font-bold text-zinc-200 transition hover:border-sky-400/60 disabled:cursor-not-allowed disabled:opacity-50">
              Run All Signals
            </button>
          </div>
